...
// Optimiser les activités existantes
function migrateExistingData(): void {
  const oldDbPath = path.join(__dirname, '..', 'data', 'activities.json');
  
  if (fs.existsSync(oldDbPath)) {
    console.log('Migrating existing data to optimized format...');
    
    try {
      const oldData = JSON.parse(fs.readFileSync(oldDbPath, 'utf-8'));
      const athletesDb: AthletesDatabase = { athletes: [], lastUpdated: new Date().toISOString() };
      const activitiesDb: OptimizedDatabase = {
        clubId: oldData.clubId || '1452985',
        activities: [],
        lastUpdated: new Date().toISOString()
      };
      
      // Traiter chaque activité
      oldData.activities.forEach((activity: any) => {
        if (activity.athlete) {
          // Ajouter l'athlète à la base d'athlètes
          ...
          // Ajouter l'activité optimisée
          const optimizedActivity: OptimizedActivity = {
            id: activity.id,
          ...
          activitiesDb.activities.push(optimizedActivity);
        }
      });
      ...
      console.log(`- ${athletesDb.athletes.length} athletes migrated`);
      console.log(`- ${activitiesDb.activities.length} activities optimized`);
